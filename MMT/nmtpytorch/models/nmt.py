...
class NMT(nn.Module):
    ...
    def set_defaults(self):
        self.defaults = {
            ...
            'dropout_emb_enc': 0,           # Simple dropout to source embeddings
            'dropout_emb_dec': 0,           # Simple dropout to source embeddings
            ...
            'dropout_dec': 0,           # Intra-encoder dropout if n_encoders > 1
            ...
            'bucket_order': None,       #
            'imagination': False,       #
            'loss_imagination': "cos",  #
            'imagination_factor': 0.1,  #
    ...
    def setup(self, is_train=True):
        ...
        self.enc = TextEncoder(
            ...
            dropout_emb=self.opts.model['dropout_emb_enc'],
        ...
        self.dec = ConditionalDecoder(
            ...
            dropout_emb=self.opts.model['dropout_emb_enc'],
            variational_dropout=self.opts.model['variational_dropout'],
            emb_maxnorm=self.opts.model['emb_maxnorm'],
            emb_gradscale=self.opts.model['emb_gradscale'],
            imagination=self.opts.model['imagination'],
            loss_imagination=self.opts.model['loss_imagination'],
            imagination_factor=self.opts.model['imagination_factor'],
            gradient_penality=self.opts.model['gradient_penality'],
            critic=self.opts.model['critic'],
            image_att=self.opts.model['image_att'],
    ...
    def forward(self, batch, **kwargs):
        ...
        """
        # Get loss dict
        result = self.dec(self.encode(batch), batch[self.tl], kwargs)
